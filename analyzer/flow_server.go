...
func NewFlowServerWebSocketConn(server *shttp.Server) (*FlowServerWebSocketConn, error) {
	flowsMax := config.GetConfig().GetInt("analyzer.storage.flow.max_flow_buffer_size")
...
func NewFlowServer(s *shttp.Server, g *graph.Graph, store storage.Storage, probe *probe.ProbeBundle) (*FlowServer, error) {
	...
	bulk := config.GetInt("analyzer.storage.flow.bulk_insert")
	bulkDeadLine := config.GetInt("analyzer.storage.flow.bulk_insert_deadline")
	if bulkDeadLine < 1 {
