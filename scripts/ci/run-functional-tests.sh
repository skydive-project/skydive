...
make test.functionals.batch GOFLAGS="$GOFLAGS" GORACE="history_size=5" WITH_MUTEX_DEBUG=true WITH_EBPF=true WITH_K8S=true VERBOSE=true TIMEOUT=20m COVERAGE=$COVERAGE ARGS="$ARGS -graph.output ascii -standalone" TEST_PATTERN=$TEST_PATTERN 2>&1 | tee $WORKSPACE/output.log
