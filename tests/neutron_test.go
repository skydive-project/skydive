...
func TestNeutron(t *testing.T) {
	...
	if err := retry.Do(fnc, retry.Attempts(60), retry.Delay(2*time.Second), retry.DelayType(retry.FixedDelay)); err != nil {
		t.Fatalf(err.Error())
	}
	...
	// test histo mode
	histo = true
	if err := retry.Do(retryFn, retry.Attempts(12)); err != nil {
		t.Error(err)
	}
}
