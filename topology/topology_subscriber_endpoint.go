...
func (t *TopologySubscriberEndpoint) OnConnected(c ws.WSSpeaker) {
...
func (t *TopologySubscriberEndpoint) OnDisconnected(c ws.WSSpeaker) {
	t.Lock()
...
func (t *TopologySubscriberEndpoint) OnWSStructMessage(c ws.WSSpeaker, msg *ws.WSStructMessage) {
...
func (t *TopologySubscriberEndpoint) notifyClients(msg *ws.WSStructMessage) {
	for _, c := range t.pool.GetSpeakers() {
		...
		if found {
			...
			for _, n := range addedNodes {
				c.SendMessage(ws.NewWSStructMessage(graph.Namespace, graph.NodeAddedMsgType, n))
			}

			for _, n := range removedNodes {
				c.SendMessage(ws.NewWSStructMessage(graph.Namespace, graph.NodeDeletedMsgType, n))
			}

			for _, e := range addedEdges {
				c.SendMessage(ws.NewWSStructMessage(graph.Namespace, graph.EdgeAddedMsgType, e))
			}

			for _, e := range removedEdges {
				c.SendMessage(ws.NewWSStructMessage(graph.Namespace, graph.EdgeDeletedMsgType, e))
			}
...
func (t *TopologySubscriberEndpoint) OnNodeUpdated(n *graph.Node) {
	t.notifyClients(ws.NewWSStructMessage(graph.Namespace, graph.NodeUpdatedMsgType, n))
}
...
func (t *TopologySubscriberEndpoint) OnNodeAdded(n *graph.Node) {
	t.notifyClients(ws.NewWSStructMessage(graph.Namespace, graph.NodeAddedMsgType, n))
}
...
func (t *TopologySubscriberEndpoint) OnNodeDeleted(n *graph.Node) {
	t.notifyClients(ws.NewWSStructMessage(graph.Namespace, graph.NodeDeletedMsgType, n))
}
...
func (t *TopologySubscriberEndpoint) OnEdgeUpdated(e *graph.Edge) {
	t.notifyClients(ws.NewWSStructMessage(graph.Namespace, graph.EdgeUpdatedMsgType, e))
}
...
func (t *TopologySubscriberEndpoint) OnEdgeAdded(e *graph.Edge) {
	t.notifyClients(ws.NewWSStructMessage(graph.Namespace, graph.EdgeAddedMsgType, e))
}
...
func (t *TopologySubscriberEndpoint) OnEdgeDeleted(e *graph.Edge) {
	t.notifyClients(ws.NewWSStructMessage(graph.Namespace, graph.EdgeDeletedMsgType, e))
}

// NewTopologySubscriberEndpoint returns a new server to be used by external subscribers,
// for instance the WebUI.
func NewTopologySubscriberEndpoint(pool ws.WSStructSpeakerPool, g *graph.Graph, tr *traversal.GremlinTraversalParser) *TopologySubscriberEndpoint {
