...
generate_cover_data() {
    ...
    if [ "$functionals" -eq 1 ]; then
        ...
        make test.functionals.batch VERBOSE=true TIMEOUT=20m GOFLAGS="-cover -covermode=$mode -coverpkg=$PKG" ARGS="$ARGS -test.coverprofile=$coverfile -standalone -analyzer.storage.topology.backend $backend -analyzer.storage.flow.backend $backend" TEST_PATTERN=$TEST_PATTERN
    fi
