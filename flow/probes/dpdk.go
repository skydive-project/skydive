...
func (p *DPDKProbesHandler) RegisterProbe(n *graph.Node, capture *api.Capture, ft *flow.Table, e FlowProbeEventHandler) error {
...
func packetHandler(packets []*packet.Packet, next []bool, nbPackets uint, context dpdkflow.UserContext) {
	...
	for i := uint(0); i < nbPackets; i++ {
		...
		if flowPackets := flow.PacketsFromGoPacket(&packet, 0, -1, nil); len(flowPackets.Packets) > 0 {
			ctx.packetsChan <- flowPackets
		}
		next[i] = false
	}
}
