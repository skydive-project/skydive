...
func (h *TPacket) getTPacketHeader() header {
	...
	case TPacketVersion1:
		...
		return (*v1header)(unsafe.Pointer(uintptr(h.ring) + uintptr(h.opts.frameSize*h.offset)))
	case TPacketVersion2:
		if h.offset >= h.opts.framesPerBlock*h.opts.numBlocks {
			h.offset = 0
		}
		return (*v2header)(unsafe.Pointer(uintptr(h.ring) + uintptr(h.opts.frameSize*h.offset)))
	case TPacketVersion3:
		...
		h.v3 = initV3Wrapper(unsafe.Pointer(uintptr(h.ring) + uintptr(h.opts.frameSize*h.offset*h.opts.framesPerBlock)))
		return &h.v3
	}
